...
#Automating the OCR tool so the code extracts the text from image itself
...
class OCRProcessor:
    ...
    def perform_ocr(self, image_path):
        try:
            ...
            text = self.tool.image_to_string(img, builder=self.builder)
            return text
...
def perform_ocr_batch(image_paths):
    ...
    for path in image_paths:
        text = ocr_processor.perform_ocr(path)
        if text:
            results[path] = text
    return results
